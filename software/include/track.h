...
class Track {
   public:
     ...
     * \param  pan  Value to be set, where -100 means 100%L and 100 means 100%R
     */
   ...
   protected:
    bool mute_state_;                //!< Mute ON/OFF
    bool solo_state_;                //!< Solo ON/OFF
    bool arm_state_;                 //!< Arm record ON/OFF
    uint8_t number_;                 //!< Track number
    uint8_t fader_level_;            //!< Fader volume level
    int8_t pan_;                     //!< Pan configuration
    uint8_t meter_;                  //!< Volume Meter
